...
int main() {
    ...
    dijkstra<pair<int, int>> pq;
    dist[1] = 0;
    pq.push(MP(0, 1));
    while (!pq.empty()) {
        int now = pq.top().ss;
        int currenttime = pq.top().ff;
        pq.pop();
        if (currenttime > dist[now])
            continue;
        for (pii i : v[now]) {
            int to = i.ff;
            int starttime = i.ss;
            int triptime = abs(now - to) * 2;
            int waittime;
            if (currenttime <= starttime) {
                waittime = starttime - currenttime;
            } else {
                waittime = (triptime - (currenttime - starttime) % triptime) % triptime;
            }
            int cost = waittime + currenttime + triptime / 2;
            if (cost < dist[to]) {
                dist[to] = cost;
                pq.push(MP(cost, to));
            }
