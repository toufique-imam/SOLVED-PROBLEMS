...
int student[29];//mask 1 means ok
ll dp[21][(1<<20)+9];
int visited[21][(1<<20)+9];
int n,x,cas;
/// mask 1 means taken

ll recur(int idx,int mask) {
    if(idx>=n)
        return 1;
    ll &ans=dp[idx][mask];
    int &vis=visited[idx][mask];
    if(vis==cas)
        return ans;
    vis=cas;
    ans=0;
    ///reverse the mask then 1 means not taken / and with student preference to get the available mask
    int pos=student[idx]&(~mask);
    while(pos) {
        int p=pos & -pos;
        pos-=p;
        ans+=recur(idx+1,mask|p);
    }
    return ans;
}
