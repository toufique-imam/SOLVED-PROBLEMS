...
ll nth_term(ll __nth,ll __dif=2,ll __first=1) {
    return (__nth-1)*__dif+__first;
}
ll sumx(ll x) {
    ll xx=nth_term(x);
    return sumy(xx)-2ll*sumy(xx/2);
}
ll sumx(ll __nth,int __dif,int __first=1) {
    return 2ll*__first+(__nth-1)*__dif;
}
ll bs1(ll x) {
    ll lo=1,hi=1e9;
    ll mid,ans=-1;
...
pll bs2(ll __x,ll __nth) {
    ll st=sumx(__nth-1)+1;
    ll lo=st,hi=sumx(nth_term(__nth));
    ll xx=__x-st+1;
    ll yy=xx/(__nth);
    pll ans;
    if(__nth%2==0) {
        //row bar e then column kom e
        if(yy) {
            //full row bere gese
            ans.ff=__nth;
            ans.ss=__nth-(xx-__nth);
        } else {
            ans.ss=__nth;
            ans.ff=(xx);
        }
    } else {
        //column bare then row kom e
        if(yy==0) {
            //full row bere gese
            ans.ff=__nth;
            ans.ss=(xx);
