...
int limit,cas;
ll dp[13][100][(1<<12)+3];
int visited[13][100][(1<<12)+3];
ll recur(int idx,int taken,int mask){
    if(idx>=n)return taken==k;
    if(taken>k)return 0;
    ll &ans=dp[idx][taken][mask];
    int &vis=visited[idx][taken][mask];
    if(vis==cas)return ans;
    vis=cas;
    ans=0;
    for(int i=0;i<n;i++){
        int cnt=0;
        ...
        if((mask&(1<<i))==0){
            ans+=recur(idx+1,taken+cnt,mask|(1<<i));
        }
    }
    return ans;
}
