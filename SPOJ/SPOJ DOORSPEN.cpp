...
void latex_line(pt x,pt y,string col="red") {
    printf("\\draw[thick](%d,%d) to (%d,%d);\n",(int)x.x,(int)x.y,(int)y.x,(int)y.y);
    printf("\\draw [%s,fill] (%d,%d) circle [radius=0.1] node [black,below=4] {(%d,%d)};\n",col.c_str(),(int)x.x,(int)x.y,(int)x.x,(int)x.y);
    printf("\\draw [%s,fill] (%d,%d) circle [radius=0.1] node [black,below=4] {(%d,%d)};\n",col.c_str(),(int)y.x,(int)y.y,(int)y.x,(int)y.y);
}
void latex_polygon(vector<pt>&x,string col="red") {
    printf("\\draw[thick]");
    for(int i=0; i<(int)x.size(); i++) {
        if(i)
            printf(" to ");
        printf("(%d,%d)",(int)x[i].x,(int)x[i].y);
    }
    printf(";\n");
    for(int i=0; i<(int)x.size()-1; i++)
        printf("\\draw [%s,fill] (%d,%d) circle [radius=0.1] node [black,below=4] {(%d,%d)};\n",col.c_str(),(int)x[i].x,(int)x[i].y,(int)x[i].x,(int)x[i].y);
}
...
int main() {
    ...
    while(scanf("%d %d",&n,&m)==2 && (n+m)>0) {
        ...
        for(int i=0; i<v1.size() && !f; i+=2) {
            for(int j=0; j<v2.size(); j+=2) {
                if(inters(v1[i],v1[i+1],v2[j],v2[j+1])) {
                    f=1;
                    break;
                }
            }
        ...
        for(int i=0; i<s2; i++)
            v2.push_back(H[i]);
        for(int i=1; i<s1 && !f; i++) {
            for(int j=1; j<s2; j++) {
                if(inters(v1[i-1],v1[i],v2[j-1],v2[j])) {
                    f=1;
                    break;
                }
            }
        ...
        for(int i=0; i<s1 && !f; i++) {
            if(pointInPolygon(v2,v1[i])) {
                f=1;
                break;
            }
        }
        for(int i=0; i<s2 && !f; i++) {
            if(pointInPolygon(v1,v2[i])) {
                f=1;
                break;
            }
        }
