...
bool check(int x,int y,int mx ,int my){
    if(x>=0 && y>=0 && x<n && y<m && grid[x][y]!=-1 && level[mx][my][x][y]==-1)
        return 1;
    return 0;
}
    ...
    while(!q.empty()){
        ...
        for(int i=0;i<4;i++){
            ...
            if(check(px,py,x,y)){
                q.push(px);
                q.push(py);
                level[x][y][px][py]=level[x][y][tx][ty]+1;
            }
        }
    }
}
...
int tsp(int idx,int mask){
    ...
    for(int i=0;i<dirty.size();i++){
        int px=dirty[i].ff;
        int py=dirty[i].ss;
        if((mask&(1<<i))==0 && level[x][y][px][py]!=-1 && i!=idx){
            int t=level[x][y][px][py]+tsp(i,mask|(1<<i));
            ans=min(ans,t);
        }
    }
    return ans;
