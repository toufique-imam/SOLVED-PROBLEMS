...
void build(int node,int st,int en) {
    ...
    build(lt,st,mid);
    build(rg,mid+1,en);
    segtree[node]=min(segtree[lt],segtree[rg]);
}
ll minq(int node,int st,int en,int l,int r) {
    if(st>=l && en<=r)
        return segtree[node];
    if(st>r || en<l)
        return 1e9+9;
    ...
    int lt=node*2;
    int rg=lt+1;
    return min(minq(lt,st,mid,l,r),minq(rg,mid+1,en,l,r));
}
int main() {
    int n;
    int cas=1;
    while(scanf("%d",&n)==1 && n>0) {
        ...
        int cc;
        for(int i=2; i<=n; i++) {
            if(ara[i]>prev && ara[i]>0)
                idx++;
            else if(ara[i]<prev && ara[i]>0) {
                cc=mp[ara[i]];
                if(cc==0)
                    idx++;
                else {
                    ll val=minq(1,1,n,cc,i);
                    if(val<ara[i])
                        idx++;
                }
            }
