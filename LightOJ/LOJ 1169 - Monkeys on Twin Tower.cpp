...
int main() {
    QUERY{
        ...
        for(int i=2;i<=n;i++){
            for(int k=0;k<2;k++){
                //ami jei builidng e asi oitar nich thke asci
                int t1=dp[0][i-1];
                if(k!=0)t1+=jump[0][i-1];  /// mane ami 1 theke 0 te asci
                int t2=dp[1][i-1];
                if(k!=1) t2+=jump[1][i-1];  /// mane ami 0 theke 1 e asci
                dp[k][i]=timex[k][i]+min(t1,t2);
            }
        }
        printf("Case %d: %d\n",_T,min(dp[0][n],dp[1][n]));
    }
}
