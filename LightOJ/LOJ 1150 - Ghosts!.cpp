...
void bfs(int Tx, int Ty) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (dist[cnt][px][py] != -1) continue;
            dist[cnt][px][py] = dist[cnt][vx][vy] + 1;
            q.push(px);
            q.push(py);
        }
    }
...
int main() {
    ...
    QUERY {
        ...
        int source = v1.size() + v2.size() + 1, sink = v1.size() + v2.size() + 2;
        ll lo = 0, hi = 1e9, ans = -1, mid;
        while (lo <= hi) {
            mid = (lo + hi) / 2;
            mf.init(v1.size() + v2.size() + 6);
           // cerr << mid << "\n";
            for (int i = 0; i < v1.size(); i++) {
                mf.addEdge(source, i, 1, 0);
                for (int j = 0; j < v2.size(); j++) {
                    if (dist[i][v2[j].first][v2[j].second] == -1) continue;
                    if (dist[i][v2[j].first][v2[j].second] + dist[i][v2[j].first][v2[j].second]+2 > mid) continue;
                    mf.addEdge(i, v1.size() + j, 1, dist[i][v2[j].first][v2[j].second] + dist[i][v2[j].first][v2[j].second] + 2);
                   // cerr << v1[i].first << " " << v1[i].second << " " << v2[j].first << " " << v2[j].second << " " << dist[i][v2[j].first][v2[j].second] + dist[i][v2[j].first][v2[j].second] + 2 << "\n";
                }
            }
            for (int j = 0; j < v2.size(); j++) {
                mf.addEdge(v1.size() + j, sink, 1, 0);
            }
            pll tmp = mf.maxFlow(source, sink);
            //cerr << tmp.first << " " << tmp.second << "\n";
            if (tmp.first == v2.size()) {
                ans = mid;
                hi = mid - 1;
            } else {
                lo = mid + 1;
