...
void merge_(ND& A_, ND& B_, ND& C_) {
    A_.lval=B_.lval;
    A_.rval=C_.rval;
    if(B_.lval==C_.lval)
        A_.lcnt=B_.lcnt+C_.lcnt;
    else
        A_.lcnt=B_.lcnt;
    if(B_.rval==C_.rval)
        A_.rcnt=B_.rcnt+C_.rcnt;
    else
        A_.rcnt=C_.rcnt;

    A_.tcnt=max(B_.tcnt,C_.tcnt);
    if(B_.rval==C_.lval)
        A_.tcnt=max(B_.rcnt+C_.lcnt,A_.tcnt);
}
void init(int node,int st,int en) {
    if(st==en) {
        stree[node].lcnt=stree[node].rcnt=stree[node].tcnt=1;
        stree[node].lval=stree[node].rval=ara[st];
        return;
    }
    int mid=(st+en)/2;
    int lt=node*2;
    ...
    init(lt,st,mid);
    init(rg,mid+1,en);
    merge_(stree[node],stree[lt],stree[rg]);
}
ND query(int node,int st,int en,int l,int r) {
    if(st>=l && en<=r)
        return stree[node];
    ...
    if(r<=mid)
        ...
        ND res,res1=query(lt,st,mid,l,mid);
