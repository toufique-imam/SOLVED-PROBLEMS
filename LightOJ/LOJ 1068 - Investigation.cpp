...
ll dp[2][12][109][109];
int visited[2][12][109][109];
string s;
int cas;
int k;
ll recur(int idx,bool islow,int sum,int mod) {
    if(idx>=s.size())
        return (mod==0 && sum==0);
    ll &ans=dp[islow][idx][sum][mod];
    int &vis=visited[islow][idx][sum][mod];
    if(vis==cas)
        return ans;
    vis=cas;
    ans=0;
    ...
    for(int i=0; i<=limit; i++)
        ans+=recur(idx+1,islow||(i<(s[idx]-'0')),(sum+i)%k,(mod*10+i)%k);
    return ans;
}
...
int main() {
    QUERY{
        ...
        for(int i=0; i<=(s[0]-'0'); i++)
            ansl+=recur(1,i<(s[0]-'0'),i%k,i%k);
        cas++;
        s=to_string(b);
        ll ansr=0;
        for(int i=0; i<=(s[0]-'0'); i++)
            ansr+=recur(1,i<(s[0]-'0'),i%k,i%k);
        printf("Case %d: %lld\n",_T,ansr-ansl);
    }
}
