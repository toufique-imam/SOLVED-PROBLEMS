...
bool check(int x){
    ...
    while(idx_now<n){
        cnt++;
        int idx=srch_idx(cum[idx_now]+x);
        if(idx<=idx_now)return 0;
...
void print_ans(int x){
    int idx_now=0,idx;
    int cc=0;
    vector<pii>v,final_ans;
    for(;idx_now<n;){
        idx=srch_idx(cum[idx_now]+x);
        cc++;
        v.EB(MP(idx_now,idx));
        idx_now=idx;
    }
    while(cc<k && v.empty()==false){
        idx_now=v.back().ff;
        idx=v.back().ss;
        v.pop_back();
        while(cc<k && idx-idx_now>1){
            final_ans.EB(MP(idx-1,idx));
            cc++;
            idx--;
        }
        final_ans.EB(MP(idx_now,idx));
    }
    while(v.empty()==false){
        final_ans.EB(v.back());
        v.pop_back();
    ...
    for(int i=0;i<final_ans.size();i++){
        printf("%d\n",cum[final_ans[i].ss]-cum[final_ans[i].ff]);
    }
}
int main() {
    QUERY{
