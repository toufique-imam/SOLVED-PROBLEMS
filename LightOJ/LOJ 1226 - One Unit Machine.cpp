...
inline ll nCr(int x, int y)
{
    return factorial[x] * (mod_inv[y] * mod_inv[x - y] % mod) % mod;
}
...
void init()
{
    ...
    mod_inv[N-1]=bigmod(factorial[N-1],mod-2);
    for(ll i=N-1; i>0; --i)
        mod_inv[i-1]=(i*1LL*mod_inv[i])%mod;
}
int ara[1009];
int main(){
    init();
    QUERY{
        ...
        for(int i=0;i<n;i++){
            scanf("%d",&ara[i]);
            sum+=ara[i];
            r=(r*mod_inv[ara[i]-1])%mod;
            rr=(rr*sum)%mod;
        }
        ll xx=bigmod(rr,mod-2);
