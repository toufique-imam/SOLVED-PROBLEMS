...
double dp[509][509][2];
bool visited[509][509][2];

double rec(int r,int b,int turn) {
    if(b==1 && r==0)
        return 1.0;
    double &ans=dp[r][b][turn];
    if(visited[r][b][turn])return ans;
    visited[r][b][turn]=true;
    ans=0;
    if(turn==0) {
        double total=r+b;
        double probr=r*1.0/total;
        double probb=b*1.0/total;
        ans=probb*rec(r,b-1,1);
        if(r>0)
            ans+=probr*rec(r-1,b,1);
        return ans;
    }
    else{
        if(b>0)
            return ans=rec(r,b-1,0);
        else return ans=0.0;
    }
}
int main() {
