...
int ara[109];
int n;
int rec(int idx,int lt,int rg){
    if(idx>n)return 0;
    int &ans=dp[idx][lt][rg];
    if(ans!=-1)return ans;
    ans=0;
    if(ara[lt]<=ara[idx] && ara[rg]>=ara[idx]){
        ans=max(ans,1+rec(idx+1,idx,rg));
    }
    if(ara[rg]>=ara[idx] && ara[lt]<=ara[idx]){
        ans=max(ans,1+rec(idx+1,lt,idx));
    }
    ans=max(ans,rec(idx+1,lt,rg));
    return ans;
}
int main(){
    QUERY{
