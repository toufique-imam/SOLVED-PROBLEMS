...
void build(int node,int st,int en) {
    ...
    if(cum_[basex][segtree[node*2]]<=cum_[basex][segtree[node*2+1]])
        segtree[node]=segtree[node*2];
    else
        segtree[node]=segtree[node*2+1];
}
ll rmq(int node,int st,int en,int l,int r) {
    if(st>=l && en<=r)
        return segtree[node];
    if(r<st || en<l)
        return -1;
    int mid=(st+en)/2;
    int pos1=rmq(node*2,st,mid,l,r);
    int pos2=rmq(node*2+1,mid+1,en,l,r);
    if(pos1==-1)
        return pos2;
    if(pos2==-1)
        return pos1;
    if(cum_[basex][pos1]<=cum_[basex][pos2])
        return pos1;
    return pos2;
}
ll recur(int l,int r) {
    ...
    if(l==r)
        return cum_[basex][l];
    int pos=rmq(1,1,m,l,r);
    ll ans=cum_[basex][pos]*(r-l+1);
    ans=max(ans,recur(l,pos-1));
    ans=max(ans,recur(pos+1,r));
    return ans;
}
ll histogram(int base) {
    basex=base;
    build(1,1,m);
    return recur(1,m);
}
        ...
        for(int base=1; base<=n; base++) {
            for(int j=0; j<m; j++) {
                if(grid[base][j]=='1')
                    ...
                    cum_[base][j+1]=1+cum_[base-1][j+1];
            }
