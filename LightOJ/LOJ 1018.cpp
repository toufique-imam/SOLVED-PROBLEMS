...
int recur(int mask=0) {
    ...
    for(int j=0;j<n;j++){
        if(pos!=j && !(mask&(1<<j))){
            ans=min(ans,1+recur(mask|pre[pos][j]));
        }
    }
    ans=min(ans,recur(mask));
    return ans;
}
int main() {
    QUERY{
        ...
        for(int i=0; i<n; i++) {
            for(int j=0; j<n; j++) {
                ...
                for(int k=0; k<n; k++) {
                    if(k==i || k==j)
                        continue;
                    if((ara[i].ff-ara[j].ff)*(ara[k].ss-ara[i].ss)==(ara[k].ff-ara[i].ff)*(ara[i].ss-ara[j].ss))
                        set(pre[i][j],k);
                }
            }
        }
