...
bool prime_check(int x) {
    ...
    if(x%2==0)
        return 0;
    if(!_check(isprime[x/32],x%32))
        return 1;
    return 0;
}
bool anagram_prime_check(int id) {
    string s=to_string(prime[id]);
    sort(all(s));
    bool f=1;
    int val=0;
    do {
        val=get_val(s);
        f=prime_check(val);
    } while(next_permutation(all(s)) && f);
    return f;
}
vector<int>anagram;
int main() {
    seive();
    for(int i=0; i<prmsz; i++) {
        if(anagram_prime_check(i))
            anagram.emplace_back(prime[i]);
    }
    int n;
    while(scanf("%d",&n)==1 && n) {
        int r=(int)pow(10, (int)log10(n)+1);
        //cerr<<r<<endl;
        int len=in_range(anagram,r,n+1);
        if(len==0)
            printf("0\n");
        else {
            auto x=lower_bound(all(anagram),n+1);
            if(*x<r)
                printf("%d\n",*x);
            else
                printf("0\n");
