...
int main()
{
    ll n, c, x, y, g, tmpx, tmpy;
    int _T = 1;
    while (scanf("%lld", &n) == 1 && n)
    {
        map<pll, vector<pair<pll, ll>>> mp[4];
        for (ll i = 0; i < n; i++)
        {
            ...
            tmpy = y / g;
            if (x >= 0 && y >= 0)
                mp[0][MP(tmpx, tmpy)].push_back(MP(MP(x, y), c));
            else if (x < 0 && y >= 0)
                mp[1][MP(tmpx, tmpy)].push_back(MP(MP(x, y), c));
            else if (x < 0 && y < 0)
                mp[2][MP(tmpx, tmpy)].push_back(MP(MP(x, y), c));
            else if (x >= 0 && y < 0)
                mp[3][MP(tmpx, tmpy)].push_back(MP(MP(x, y), c));
        }
        vector<pll> ans;
        for (int k = 0; k < 4; k++)
        {
            for (auto &i : mp[k])
            {
                sort(all(i.ss), [](const pair<pll, ll> &a, const pair<pll, ll> &b) {
                    ll xx = a.ff.ff * a.ff.ff + a.ff.ss * a.ff.ss;
                    ll yy = b.ff.ff * b.ff.ff + b.ff.ss * b.ff.ss;
                    if (xx != yy)
                        return xx < yy;
                    else if (a.ff != b.ff)
                        return a.ff < b.ff;
        ...
        else
        {
            ...
            {
                printf("x = %lld, y = %lld", ans[i].ff, ans[i].ss);
                if (i == ans.size() - 1)
                    printf(".\n");
                else
                    printf(";\n");
