...
void build(int node,int st,int en){
    ...
    build(lt,st,mid);
    build(rg,mid+1,en);
    segment[node]=min(segment[lt],segment[rg]);
}
void update(int node,int st,int en,int idx,int val){
    ...
    int rg = lt + 1;
    int mid = (st + en) / 2;
    update(lt,st,mid,idx,val);
    update(rg,mid+1,en,idx,val);
    segment[node]=min(segment[lt],segment[rg]);
}
int query(int node,int st,int en,int l,int r){
    if(st>r || en<l)return 1e9;
    if(st>=l && en<=r)return segment[node];
    int lt=node*2;
    int rg=lt+1;
    int mid=(st+en)/2;
    int q1=query(lt,st,mid,l,r);
    int q2=query(rg,mid+1,en,l,r);
    return min(q1,q2);
}
int main()
{
    ...
    while(q--){
        ...
        process(s);
        if(s[0]=='q'){
            cout<<query(1,1,n,qq[0],qq[1])<<"\n";
        }else{
            int xx=qq.size();
            for(int i=0;i<xx;i++){
                update(1,1,n,qq[i],val[(i+1)%xx]);
                ara[qq[i]]=val[(i+1)%xx];
            }
        }
    }
