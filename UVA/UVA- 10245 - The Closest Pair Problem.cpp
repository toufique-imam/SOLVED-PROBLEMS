...
public:
    ...
    circle(point a,point b,point c)
    {
        ...
        double g=0.5*(x1+x2+C*(y1-y2));
        double f=0.5*(y1+y2-C*(x1-x2));
        double mc=x1*x2+y1*y2+C*x1*(y1-y2)-C*y1*(x1-x2);
...
int main()
{
    ...
    while(cin>>n and n){
        ...
        sort(ara,ara+n);
        double maxi=10001.0,tmp,tmp2;
        for(int i=0;i<n;i++){
            //tmp=sqrtl(distance_(ara[i],ara[i+1]));
            for(int j=i+1;j<n;j++){
                tmp=sqrtl(distance_(ara[i],ara[j]));
                if(maxi<fabs(ara[i].ff-ara[j].ff))break;
                if(maxi>tmp)maxi=tmp;
            }
        }
        if(maxi<10000.0){
            cout<<fixed<<setprecision(4)<<maxi<<endl;
        }
