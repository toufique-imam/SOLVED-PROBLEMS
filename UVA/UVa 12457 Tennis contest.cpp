...
int visited[100][100];
int mark;
double recur(int total,int won){
    if(won==n)return 1.0;
    if(total==2*n-1)return 0.0;
    if(visited[total][won]==mark)return dp[total][won];
    visited[total][won]=mark;
    return dp[total][won]=p*recur(total+1,won+1)+(1-p)*recur(total+1,won);
