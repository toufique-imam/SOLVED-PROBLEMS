...
const int N=209;
string s1,s2;
int supersink=150,supersource=0;
int grid[N][N];
vector<int>v[N];
int par[N],mini[N];
int getmap(string s) {
...
int bfs(){
    ...
    while(!q.empty()){
        t=q.front();
        q.pop();
        if(t==supersink)return mini[supersink];
...
void update_path(int x,int val){
    if(par[x]==x || par[x]==-1)return;
    grid[par[x]][x]-=val;
    grid[x][par[x]]+=val;
    update_path(par[x],val);
}
int maxflow(){
    int ans=0;
    int mf=bfs();
...
int main() {
    QUERY{
        ...
        for(int i=0; i<m; i++) {
            cin>>s1>>s2;
            volnow=cnt++;
            assert(volnow<supersink);
            grid[getmap(s1)][volnow]=1;
            grid[getmap(s2)][volnow]=1;
            grid[volnow][supersink]=1;

            v[getmap(s1)].EB(volnow);
            v[getmap(s2)].EB(volnow);
            v[volnow].EB(supersink);

            v[volnow].EB(getmap(s1));
            v[volnow].EB(getmap(s2));
        }
        for(int i=1; i<7; i++) {
            grid[supersource][i]=n/6;
            v[supersource].EB(i);
        }
        int ans=maxflow();
        if(ans==m){
