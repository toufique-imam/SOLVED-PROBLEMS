...
{
    node C;
    for(int i=0; i<3; i++)C.cnt[i]=A.cnt[i]+B.cnt[i];
...
void free_lazy(int par,int st,int en)
{
    ...
    if(tree[par].lazy%3==1)
    {
        ...
        for(int i=2; i>0; i--)tree[par].cnt[i]=tree[par].cnt[i-1];
        tree[par].cnt[0]=x;
    }
    else if(tree[par].lazy%3==2)
    {
        int x=tree[par].cnt[0];
        for(int i=0; i<2; i++)tree[par].cnt[i]=tree[par].cnt[i+1];
        tree[par].cnt[2]=x;
    }
    if(h>1)push_down(par);
    tree[par].lazy=0;
    return;
}
void update(int par,int st,int en,int L,int R)
{
    if(tree[par].lazy)free_lazy(par,st,en);
